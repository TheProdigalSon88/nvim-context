...
---Opens a persistent, editable note pane that tracks quickfix navigation.
---
---The pane shows the description (and code block) for the current quickfix
---entry. It updates whenever the user navigates with :cnext/:cprev/:cc etc.
---from any buffer. The matched range is highlighted in the source buffer using
---the Visual highlight group. Saving with :w writes the edited description back
---via save_item.
---
---@param save_item fun(item: vim.quickfix.entry, description: string)
function Buffer.open_qf_note_viewer(save_item)
   local buf = vim.api.nvim_create_buf(false, false)
   vim.bo[buf].buftype = "acwrite"
   vim.bo[buf].bufhidden = "wipe"
   vim.bo[buf].swapfile = false
   vim.bo[buf].filetype = "markdown"
   vim.api.nvim_buf_set_name(buf, "nvim-context-qf-note://" .. buf .. ".md")

   vim.cmd("botright vsplit")
   vim.api.nvim_win_set_buf(0, buf)
   local win = vim.api.nvim_get_current_win()

   -- Track the qf item currently displayed and the last highlighted buffer so
   -- we can clear the extmark before applying it to the next location.
   local current_item = nil
   local last_hl_buf = nil

   ---Apply Visual extmark over lnum..end_lnum in the item's source buffer.
   local function apply_highlight(item)
      if last_hl_buf and vim.api.nvim_buf_is_valid(last_hl_buf) then
         vim.api.nvim_buf_clear_namespace(last_hl_buf, hl_ns, 0, -1)
      end
      last_hl_buf = nil
      ...
      local target_buf = (item.bufnr and item.bufnr ~= 0) and item.bufnr
         or (item.filename and vim.fn.bufnr(item.filename) or nil)
      if not target_buf or target_buf == -1 or not vim.api.nvim_buf_is_valid(target_buf) then
         return
      end

      local start_row = (item.lnum or 1) - 1
      local end_row = (item.end_lnum and item.end_lnum ~= 0) and item.end_lnum or item.lnum or 1

      vim.api.nvim_buf_set_extmark(target_buf, hl_ns, start_row, 0, {
         end_row = end_row, -- extmark end_row is exclusive, so this covers through end_lnum
         hl_group = "Visual",
         hl_eol = true,
         strict = false,
         priority = 160,
      ...
      last_hl_buf = target_buf
   end

   ---Populate the note buffer with the description and code of a qf entry.
   ---Clears the buffer when item is nil or has no user_data.
   local function update(item)
      current_item = item
      local lines = {}

      if item and type(item.user_data) == "table" then
         local ud = item.user_data
         local desc = ud.description or ""
         for line in (desc .. "\n"):gmatch("(.-)\n") do
            table.insert(lines, line)
         end
         -- Trim trailing blank lines from the description section, then add one
         -- blank separator before the code block (mirrors open_note_editor layout).
         while #lines > 0 and lines[#lines] == "" do
            table.remove(lines)
         end
         table.insert(lines, "")

         local code = ud.base_text
         if code and code ~= "" then
            -- Detect language from source buffer (best-effort; bufnr may be 0).
            local lang = ""
            local src = (item.bufnr and item.bufnr ~= 0) and item.bufnr
               or (item.filename and vim.fn.bufnr(item.filename) or nil)
            if src and src ~= -1 and vim.api.nvim_buf_is_loaded(src) then
               lang = vim.bo[src].filetype or ""
            end
            table.insert(lines, "")
            table.insert(lines, CODE_DELIMITER)
            table.insert(lines, "```" .. lang)
   ...
   local augroup = vim.api.nvim_create_augroup("ContextQfNoteViewer_" .. buf, { clear = true })

   -- React to :cnext / :cprev / :cc etc. by polling the qf index on every
   -- cursor move. QuickFixCmdPost does not fire for navigation commands, so
   -- CursorMoved is the reliable hook. The idx guard ensures update() is only
   -- called when the selected entry actually changes.
   local last_qf_idx = nil
   ...
   -- Clear highlight and tear down autocmds when the note pane is closed.
   vim.api.nvim_create_autocmd("BufWinLeave", {
      group = augroup,
      buffer = buf,
      once = true,
   ...
   -- Save: strip code section, trim, persist via save_item.
   vim.api.nvim_create_autocmd("BufWriteCmd", {
      buffer = buf,
      callback = function()
         ...
         local description = table.concat(reference_lines, "\n"):gsub("^%s+", ""):gsub("%s+$", "")
         vim.bo[buf].modified = false
         save_item(current_item, description)
      end,
   })
   ...
   end, { buffer = buf, desc = "Close qf note viewer" })

   -- Eagerly populate with the currently active qf item so the buffer is not
   -- empty on open (cc 1 may be a no-op if the index is already 1, in which
   -- case QuickFixCmdPost would not fire).
   local idx = vim.fn.getqflist({ idx = 0 }).idx
   if idx and idx > 0 then
      local item = vim.fn.getqflist()[idx]
      update(item)
